...
{
    if ( c == '\n' )
    { scrx = 0; scry += LETTER_HEIGHT + SPACING; return; }

    if ( c < ' ' || c > '~' )
        return;

    for ( uint8_t line = 0; line < LETTER_HEIGHT; line++ )
    {
        uint8_t value = bitmap[c - ' '][LETTER_HEIGHT - line - 1];
        for ( uint8_t col = 0; col < LETTER_WIDTH; col++ )
        {
            inf->vmem[(scry+line)*inf->width + (scrx+LETTER_WIDTH-col)] = color * (value % 2);
            value /= 2;
        }
    }
    scrx += LETTER_WIDTH + SPACING;
}
